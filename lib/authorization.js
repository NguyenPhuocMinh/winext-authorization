...
function Authorization(params = {}) {
  ...
  this.publicPaths = async function (request, response, next) {
    try {
  ...
  this.protectedRouter = async function (request, response, next) {
    try {
      ...
      if (!enable || request.hasToken || request.isPublic) {
        ...
        if (!isEmpty(request.accessToken) && !isEmpty(findProtectedPath)) {
          ...
          if (includes(userPermissions, protectedPermission)) {
            return next();
          } else {
