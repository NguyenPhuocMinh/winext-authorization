...
function Authorization(params = {}) {
  ...
  this.noVerifyToken = async function (request, response, next) {
    try {
      ...
      loggerTracer.info(chalk.blue(`Load function noVerifyToken by ${name}-${version} successfully!`));
  ...
  this.verifyTokenMiddleware = async function (request, response, next) {
    try {
      ...
      loggerTracer.info(chalk.blue(`Load function verifyTokenMiddleware by ${name}-${version} successfully!`));
      ...
      if (!enable || request.hasToken || !enablePublicPath || !enableProtectedPath) {
        return next();
      } else {
        /**
  ...
  this.publicRouters = async function (request, response, next) {
    try {
      ...
      loggerTracer.info(chalk.blue(`Load function publicRouters by ${name}-${version} successfully!`));
  ...
  this.protectedRouters = async function (request, response, next) {
    try {
      ...
      loggerTracer.info(chalk.blue(`Load function protectedRouters by ${name}-${version} successfully!`));
      ...
      if (!enable || request.hasToken || request.isPublic) {
        ...
        if (!isEmpty(request.accessToken) && !isEmpty(findProtectedPath)) {
          ...
          loggerTracer.info(chalk.blue(`User permissions : ${userPermissions}`));
