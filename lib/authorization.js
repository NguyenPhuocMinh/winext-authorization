...
function Authorization(params = {}) {
  ...
  this.noVerifyToken = async function (request, response, next) {
    try {
      loggerTracer.info(`Method no need check token start with args`, {
        args: {
          enable,
      ...
      loggerTracer.info(`Method no need check token has been end`);
    } catch (err) {
  ...
  this.verifyTokenMiddleware = async function (request, response, next) {
    try {
      loggerTracer.info(`Check token has been start with args`, {
        args: {
          enable,
      ...
      loggerTracer.info(`Check token path has been end`);
    } catch (err) {
  ...
  this.publicRouters = async function (request, response, next) {
    try {
      loggerTracer.info(`Check public path has been start with args`, {
        args: {
          enable,
      ...
      loggerTracer.info(`Check public path has been end`);
    } catch (err) {
  ...
  this.protectedRouters = async function (request, response, next) {
    try {
      loggerTracer.info(`Check protected path has been start with args`, {
        args: {
          enable,
          hasToken: request.hasToken ? request.hasToken : false,
          accessToken: request.accessToken ? request.accessToken : null,
      ...
      if (!enable || request.hasToken || request.isPublic) {
        ...
        if (!isEmpty(request.accessToken) && !isEmpty(findProtectedPath)) {
          ...
          loggerTracer.verbose(`User permissions`, {
      ...
      loggerTracer.info(`Check protected path has been end`);
    } catch (err) {
