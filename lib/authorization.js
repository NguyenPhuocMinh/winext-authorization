  ...
  this.verifyTokenMiddleware = async function (request, response, next) {
    try {
      ...
      if (!enable || request.hasToken) {
        ...
        if (isEmpty(token)) {
          const tokenInvalidError = errorManager.newError('TokenInvalid', errorCodes);

          return response.status(tokenInvalidError.statusCode).send({
            data: {},
            method: request.method,
            endpoint: request.path,
            name: tokenInvalidError.name,
            message: tokenInvalidError.message,
            returnCode: tokenInvalidError.returnCode,
            statusCode: tokenInvalidError.statusCode,
          });
        } else {
          /**
           * verify token
